...
const bodyContent = [
  ...
  new Paragraph({ children: [new PageBreak()] }), // Page Break before Section 3
  ...
  new Paragraph({ children: [new PageBreak()] }), // Page Break before Section 4
  ...
  new Paragraph({ children: [new PageBreak()] }), // Page Break before Section 5
...
const part4_DiscussionAndReferences = [
  ...
  bodyText("The quantitative metrics reveal a clear trade-off between latency, token cost, and solution accuracy. While multi-agent pipelines require more API calls, the resulting gains in educational utility and code correctness are substantial. The modular design of our coordinator architecture makes it easy to integrate additional specialist agents (e.g., a dynamic testing sandbox agent that runs the generated code and provides feedback)."),
  ...
  bulletItem("2. Limited context recursion: Deep recursive algorithms may occasionally cause token overflow if the explanations are extremely detailed and long."),
  ...
  bodyText("Future work will focus on integrating a local sandboxed compiler to run tests on generated algorithms and using parallel execution to reduce Coordinator latency. By utilizing multiprocessing modules, we aim to reduce latency down to 1.8 seconds while using Gemini APIs."),
  new Paragraph({ children: [new PageBreak()] }), // Page Break before Section 6
...
Packer.toBuffer(doc).then((buffer) => {
  const outputPath = path.join("d:\\ali\\AI", "AI_Assignment_02_Solution_Final.docx");
