...
const {
  ...
  AlignmentType, PageBreak, SectionType, Table, TableRow, TableCell,
  WidthType
} = docx;
...
function paperAuthors() {
  return [
    new Paragraph({
      children: [
        new TextRun({ text: "Muhammad Taqi (22-SE-52)*", font: "Arial", size: 18, bold: true }),
        new TextRun({ text: ", ", font: "Arial", size: 18 }),
        new TextRun({ text: "Ali Hussnain (22-SE-100)*", font: "Arial", size: 18, bold: true })
      ],
      alignment: AlignmentType.LEFT,
      spacing: { after: 100 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: "Software Engineering Department, Faculty of Telecommunication and Information Engineering,", font: "Arial", size: 14, italics: true }),
      ],
      alignment: AlignmentType.LEFT,
    }),
    new Paragraph({
      children: [
        new TextRun({ text: "University of Engineering and Technology, Taxila", font: "Arial", size: 14, italics: true }),
      ],
      alignment: AlignmentType.LEFT,
      spacing: { after: 200 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: "*Corresponding authors. E-mail: taqi@student.uettaxila.edu.pk, ali@student.uettaxila.edu.pk", font: "Arial", size: 12 }),
      ],
      alignment: AlignmentType.LEFT,
      spacing: { after: 200 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: "Submitted to: Dr. Kanwal Yousaf", font: "Arial", size: 14, bold: true }),
      ],
      alignment: AlignmentType.LEFT,
      spacing: { after: 400 },
    }),
...
function abstractSection() {
  return [
    new Paragraph({
      children: [new TextRun({ text: "Abstract", font: "Arial", size: 18, bold: true })],
      alignment: AlignmentType.LEFT,
      spacing: { after: 120 },
    }),
    new Paragraph({
    ...
    new Paragraph({
      children: [
        new TextRun({ text: "Keywords: ", font: "Arial", size: 18, bold: true }),
        new TextRun({ text: "Artificial Intelligence, Large Language Models, Multi-Agent Systems, Algorithm Generation, Prompt Engineering, Software Engineering Education.", font: "Times New Roman", size: 20 })
      ],
      spacing: { after: 400 },
    }),
  ];
...
function subHeading(text) {
  return new Paragraph({
    children: [new TextRun({ text, font: "Arial", size: 16, bold: true, italics: true })],
...
function subSubHeading(text) {
  return new Paragraph({
    children: [new TextRun({ text, font: "Arial", size: 14, bold: true })],
...
function boldBodyText(label, text) {
  return new Paragraph({
    children: [
      ...
      new TextRun({ text: text, font: "Times New Roman", size: 20 }),
    ],
    spacing: { after: 120, line: 280 },
    alignment: AlignmentType.JUSTIFIED,
  });
...
const part1_Introduction = [
  ...
  boldBodyText("Target Users: ", "The primary target users include undergraduate computer science and software engineering students, competitive programmers seeking algorithmic insights, and junior software developers who need quick algorithmic references and explanations during development."),
...
const part2_MaterialAndMethods = [
  ...
  boldBodyText("Stage 2 — Query Classification: ", "The system classifies the query into one or more task categories: algorithm generation, algorithm explanation, complexity analysis, or algorithm comparison. This classification determines which prompt template to use and which agents to call."),
  ...
  boldBodyText("Stage 4 — LLM Processing: ", "The constructed prompts are sent to the designated LLM API (Google Gemini API as primary, OpenAI GPT-4 API as secondary). The models process the prompts and generate responses asynchronously."),
  ...
  new Paragraph({
    children: [
      new ImageRun({ data: pipeImg, transformation: { width: 320, height: 213 }, type: "png" }),
    ],
    alignment: AlignmentType.CENTER,
    spacing: { before: 120, after: 120 },
  }),
  new Paragraph({
    children: [new TextRun({ text: "Fig. 2: Methodology Pipeline of the AI Agent for Algorithm Generation and Explanation", italics: true, font: "Times New Roman", size: 18 })],
    alignment: AlignmentType.CENTER,
    spacing: { after: 200 },
  }),
  ...
  boldBodyText("1. Coordinator Agent: ", "Acts as the central router, parsing user inputs, calling the Classifier Agent, and then distributing sub-tasks to the Generator, Explainer, and Analyzer. It aggregates all responses into a cohesive markdown document."),
  boldBodyText("2. Classifier Agent: ", "An intent-parsing LLM agent. It scans the query to output a comma-separated list of intents (GENERATE, EXPLAIN, ANALYZE, COMPARE). This routing saves token usage by only calling the required specialist agents."),
  boldBodyText("3. Code Generator Agent: ", "Generates optimal, modular implementations in Python, C++, Java, or JavaScript. It focuses purely on syntax and code structure, skipping any conceptual discussion."),
  boldBodyText("4. Logic Explainer Agent: ", "Constructs conceptual walkthroughs and dry-runs using Chain-of-Thought (CoT) prompting to optimize student understanding."),
  boldBodyText("5. Complexity Analyzer Agent: ", "Deduces time and space bounds (Best, Average, Worst case) and writes proof-based mathematical justifications in LaTeX formats."),

  subHeading("4.4 Technical Stack"),
  bulletItem("Programming Language: Python 3.10+"),
  bulletItem("LLM API: Google Gemini API (primary) / OpenAI GPT-4 API (secondary)"),
  bulletItem("Prompt Engineering: Custom prompt templates with chain-of-thought reasoning"),
  bulletItem("Interface: Streamlit-based web interface for user interaction"),
  bulletItem("Output Formatting: Markdown rendering with syntax-highlighted code blocks"),
  ...
  bodyText("For algorithm generation tasks, the prompt template instructs the LLM to first analyze the problem requirements, then provide the algorithm in pseudocode, followed by implementation in the requested programming language, step-by-step explanation of the logic, time and space complexity analysis, and finally practical use cases and examples. By modularizing these templates, we avoid overwhelming the model with multi-task instructions in a single prompt."),
];

const part3_Experiments = [
  sectionHeading("5. Experiments and Results"),
  bodyText("We conducted quantitative evaluations to measure the performance of our multi-agent model against multiple baselines and ablation models. Evaluations were performed on a benchmark dataset of 10 classic computational problems including Dijkstra's algorithm, A* search, Fibonacci dynamic programming, Kruskal's MST, and sorting methods."),
  ...
  bodyText("The metrics evaluated include Code Correctness (%), Explanation Score (rubric graded 1 to 5), Complexity Accuracy (%), Average Latency (seconds), and Average Token count."),
];

// Recreate the evaluation data from evaluator.py
const tableRows = [
  new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Configuration", bold: true, font: "Arial", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Code Correctness (↑)", bold: true, font: "Arial", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Explanation Score (1-5) (↑)", bold: true, font: "Arial", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Complexity Accuracy (↑)", bold: true, font: "Arial", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Avg Latency (↓)", bold: true, font: "Arial", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Avg Tokens (↓)", bold: true, font: "Arial", size: 18 })] })] })
    ]
  }),
  new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Proposed Agent (Gemini 1.5 Pro)", font: "Times New Roman", size: 18, bold: true })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "100.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "4.88", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "100.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "4.51s", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2765", font: "Times New Roman", size: 18 })] })] })
    ]
  }),
  new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Proposed Agent (Gemini 1.5 Flash)", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "100.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "4.58", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "90.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "3.12s", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2621", font: "Times New Roman", size: 18 })] })] })
    ]
  }),
  new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Baseline (Zero-Shot GPT-4o mini)", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "72.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2.56", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "63.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2.37s", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "1240", font: "Times New Roman", size: 18 })] })] })
    ]
  }),
  new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Baseline (Single-Prompt Flash)", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "81.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "4.57", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "72.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2.85s", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2679", font: "Times New Roman", size: 18 })] })] })
    ]
  }),
  new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Ablation (w/o Routing Classifier)", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "54.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "4.65", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "90.0%", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2.67s", font: "Times New Roman", size: 18 })] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "2718", font: "Times New Roman", size: 18 })] })] })
    ]
  })
];
...
const part4_DiscussionAndReferences = [
  new Paragraph({
    children: [new TextRun({ text: "Table 1: Comparison of evaluation metrics across models and agent configurations. Underlined values indicate second best, while bold indicates best overall performance.", italics: true, font: "Times New Roman", size: 18 })],
    alignment: AlignmentType.CENTER,
    spacing: { before: 100, after: 150 },
  }),

  subHeading("5.2 Analysis of Quantitative Results"),
  bodyText("Our multi-agent system demonstrated a balanced, high-performing output profile. The Proposed Agent (Gemini 1.5 Pro) achieved perfect code correctness (100%) and complexity accuracy (100%), with an explanation score of 4.88 out of 5. This is a substantial improvement over the zero-shot baseline (GPT-4o mini), which achieved 72.0% correctness and 63.0% complexity accuracy, with explanations scoring 2.56. While zero-shot baseline latency was lower (2.37s) and used fewer tokens (1240), it generated incomplete or syntactically invalid code for complex graph search algorithms (e.g., A* and Dijkstra's)."),
  bodyText("The ablation model (Proposed Agent w/o Routing Classifier) showed the severe negative impact of skipping query classification. Without routing, code correctness dropped to 54.0%. This occurred because a single overloaded query forced the model to perform multiple distinct cognitive tasks (generation, explanation, analysis) in a single response context. The model often failed to structure the output properly or skipped comments, resulting in broken syntax blocks. This demonstrates that routing queries to specialized prompt wrappers is crucial for model accuracy."),

  sectionHeading("6. Discussion and Future Work"),
  bodyText("The quantitative metrics reveal a clear trade-off between latency, token cost, and solution accuracy. While multi-agent pipelines require more API calls, the resulting gains in educational utility and code correctness are substantial. The modular design of our coordinator architecture makes it easy to integrate additional specialist agents (e.g., an dynamic testing sandbox agent that runs the generated code and provides feedback)."),
  bodyText("However, several limitations exist in the current prototype:"),
  bulletItem("1. Absence of live runtime validation: The agent does not execute the generated code in a sandbox to catch rare edge-case bugs."),
  bulletItem("2. Limited context recursion: Deep recursive algorithms may occasionally cause token overflow if the explanations are extremely detailed."),
  ...
  bodyText("Future work will focus on integrating a local sandboxed compiler to run tests on generated algorithms and using parallel execution to reduce Coordinator latency."),

  sectionHeading("7. References"),
  ...
  bodyText("[10] T. Kojima, S. Gu, M. Reid, et al., 'Large Language Models Are Zero-Shot Reasoners,' Advances in Neural Information Processing Systems (NeurIPS), vol. 35, pp. 22199–22213, 2022."),
...
const part5_Appendix = [
  sectionHeading("8. Appendix: Prompt Templates and Example Executions"),
  bodyText("This appendix contains the exact prompt engineering configurations utilized in the multi-agent system, followed by a detailed case study execution walkthrough."),

  subHeading("8.1 System Prompt Templates"),
  
  subSubHeading("8.1.1 Classifier Agent System Prompt"),
  ...
  subSubHeading("8.1.2 Code Generator Agent Prompt"),
  ...
  subSubHeading("8.1.3 Logic Explainer Agent Prompt"),
  ...
  subSubHeading("8.1.4 Complexity Analyzer Agent Prompt"),
  ...
  subHeading("8.2 Agent Execution Case Study: Dijkstra's Algorithm"),
  bodyText("Below is the recorded raw input, intermediate processing states, and aggregated output for Dijkstra's Shortest Path algorithm query:"),
  ...
  sections: [
    {
      ...
      children: [
        paperTitle("AI-Powered Algorithm Generation and Explanation Agent"),
        ...paperAuthors(),
        ...abstractSection(),
      ],
    },
    {
      properties: {
        page: { margin: { top: 1440, bottom: 1440, left: 1000, right: 1000 } },
        column: { space: 400, count: 2 },
        type: SectionType.CONTINUOUS,
      },
      children: [
        ...part1_Introduction,
        ...part2_MaterialAndMethods,
        ...part3_Experiments,
        resultsTable,
...
Packer.toBuffer(doc).then((buffer) => {
  const outputPath = path.join("d:\\ali\\AI", "AI_Assignment_02_Report_Final.docx");
  fs.writeFileSync(outputPath, buffer);
  console.log("Assignment 2 Academic paper created successfully at:", outputPath);
});
